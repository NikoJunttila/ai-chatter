-- File 1: lua/ai-chatter/init.lua
local M = {}
local ui = require("ai-chatter.ui")
local chat = require("ai-chatter.chat")
local picker = require("ai-chatter.picker")
...
-- Configuration
local config = {
	templates_dir = vim.fn.stdpath("config") .. "/ai-chatter-templates",
...
function M.clear_chat()
	if #state.chat_history == 0 then
		print("Chat is already empty")
		return
	end

	local confirm = vim.fn.confirm("Clear chat history (" .. #state.chat_history .. " messages)?", "&Yes\n&No", 2)
...
function M.clear_contexts()
	if #state.context_files == 0 then
		print("No context files to clear")
		return
	end

	local confirm = vim.fn.confirm("Clear all context files (" .. #state.context_files .. " files)?", "&Yes\n&No", 2)

	if confirm == 1 then
		chat.clear_context_files(state)
		M.refresh_ui()
	end
end

-- Reset everything (chat + contexts)
function M.reset_all()
	if #state.chat_history == 0 and #state.context_files == 0 then
	...
	if confirm == 1 then
		state.chat_history = {}
		state.context_files = {}
		print("Reset complete: cleared chat and context files")
		M.refresh_ui()
	end
end
...
function M.setup(opts)
	...
	vim.api.nvim_create_user_command("ChatClearContexts", function()
		M.clear_contexts()
	end, { desc = "Clear all context files" })

	vim.api.nvim_create_user_command("ChatReset", function()
		M.reset_all()
	end, { desc = "Reset chat and contexts" })
end
